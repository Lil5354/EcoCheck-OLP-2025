...
export default function RouteOptimization() {
  ...
  async function autoFilterByDistrict() {
    ...
    if (p.ok && Array.isArray(p.data)) {
      const filtered = p.data.filter(
        (emp) => emp.role === "driver" || emp.role === "collector"
      );
  ...
  function displayRouteOnMap(route) {
    ...
    if (hasValidGeometry) {
      ...
      if (source) {
        ...
        console.log(
          `[RouteOptimization] Route displayed on map with ${coordCount} coordinates (START and END included by backend)`
        );
      } else {
      ...
      if (route.depot && route.depot.lon && route.depot.lat) {
        bounds.extend([
          parseFloat(route.depot.lon),
          parseFloat(route.depot.lat),
        ]);
      }
      ...
      console.warn("[RouteOptimization] No valid route geometry to display", {
        ...
        coordinatesCount:
          route.geojson?.features?.[0]?.geometry?.coordinates?.length || 0,
      });
    }

    // Add marker for depot (start point) - Improved styling with clear START label
    console.log(
      "[RouteOptimization] Display route - depot:",
      route.depot,
      "dump:",
      route.dump
    );
    console.log(
      "[RouteOptimization] Using NEW marker style - START (green square) and END (red circle)"
    );
    if (route.depot && route.depot.lon && route.depot.lat) {
      const depotEl = document.createElement("div");
      depotEl.className = "route-depot-marker";
      // Set all styles with !important to override any CSS
      depotEl.setAttribute(
        "style",
        `
      ...
      console.log(
        "[RouteOptimization] Added depot marker at:",
        route.depot.lon,
        route.depot.lat
      );
    } else {
      console.warn(
        "[RouteOptimization] Depot missing or invalid:",
        route.depot
      );
    }
    ...
    if (route.dump && route.dump.lon && route.dump.lat) {
      ...
      console.log(
        "[RouteOptimization] Added dump marker at:",
        route.dump.lon,
        route.dump.lat
      );
    } else {
      ...
      poiMarkersRef.current.forEach((marker) => {
        if (marker && marker.remove) {
          try {
            ...
            console.warn(
              "[RouteOptimization] Error removing POI marker:",
              error
            );
          }
        }
  ...
  async function loadPOIsForCurrentRoute() {
    ...
    try {
      ...
      console.log(
        `[RouteOptimization] Loading POIs for map center: [${center.lat}, ${
          center.lng
        }], radius: ${Math.round(searchRadius)}m`
      );
      ...
      if (parkingRes?.ok && Array.isArray(parkingRes.data)) {
        allPois.push(
          ...parkingRes.data
            .slice(0, 15)
            .map((p) => ({ ...p, type: "parking" }))
        );
      }

      console.log(
        `[RouteOptimization] Found ${allPois.length} POIs (gas: ${
          gasRes?.data?.length || 0
      ...
      if (allPois.length > 0) {
        ...
        setToast({
          message: "Không tìm thấy POI trong khu vực này",
          type: "info",
        });
      }
    } catch (error) {
    ...
    poisData.forEach((poi) => {
      ...
      try {
        ...
        el.style.cssText = `
          ...
          border-radius: 50% !important;
          background-color: ${
            poi.type === "gas_station" ? "#f59e0b" : "#6366f1"
          } !important;
          ...
          .setPopup(
            new maplibregl.Popup({ offset: 25, closeButton: true }).setHTML(`
                <div style="padding: 6px; min-width: 120px;">
                  <strong>${
                    poi.name ||
                    (poi.type === "gas_station" ? "Trạm xăng" : "Bãi đỗ xe")
                  }</strong><br/>
                  <small style="color: #666;">${
                    poi.type === "gas_station" ? "⛽" : "🅿️"
                  } ${
  ...
  async function handleOptimize() {
    ...
    if (selectedVehicles.length === 0) {
      setToast({
        message: "Vui lòng chọn ít nhất một phương tiện",
        type: "error",
      });
      return;
    }
    ...
    const payload = {
      ...
      vehicles,
      depot: depot
        ? { id: depot.id, name: depot.name, lon: depot.lon, lat: depot.lat }
        : { lon: 106.7, lat: 10.78 },
      dump: dump
        ? { id: dump.id, name: dump.name, lon: dump.lon, lat: dump.lat }
        : null,
    ...
    if (res.ok) {
      ...
      console.log(
        "[RouteOptimization] Optimized routes received:",
        optimizedRoutes.length
      );
      setRoutes(optimizedRoutes);
      setToast({
        message: `Tối ưu tuyến đường ${selectedDistrict} thành công`,
        type: "success",
      });
            ...
            <div className="card" style={{ marginBottom: 16 }}>
              ...
              >
                  ...
                  >
                    ...
                    {districts.map((d) => (
                      <option key={d.district} value={d.district}>
                        {d.district} ({d.point_count || d.schedule_count || 0}{" "}
                        điểm)
                      </option>
                    ))}
                  </select>
                ...
                >
                  <button
                    className={`btn ${
                      showPOIs ? "btn-primary" : "btn-secondary"
                    }`}
                    onClick={async () => {
                      if (!showPOIs) {
                        setShowPOIs(true);
              ...
              {selectedDistrict && (
                ...
                >
                  📍 <strong>{selectedDistrict}</strong>: Tìm thấy{" "}
                  {schedules.length} điểm thu gom · {depots.length} trạm ·{" "}
                  {fleet.length} phương tiện
                </div>
              )}
            </div>
            ...
            >
              <div className="card">
                ...
                <div style={{ marginBottom: 12, color: "#666", fontSize: 14 }}>
                  {selectedDistrict ? (
                    <>
                      Tìm thấy {schedules.length} điểm thu gom tại{" "}
                      <strong>{selectedDistrict}</strong> cho ngày{" "}
                      {new Date(collectionDate).toLocaleDateString("vi-VN")}
                    </>
                  ) : (
                    <>
                      Tìm thấy {schedules.length} lịch thu gom cho ngày{" "}
                      {new Date(collectionDate).toLocaleDateString("vi-VN")}
                    </>
                  )}
                </div>
            ...
            {routes.length > 0 && (
              <div className="card" style={{ marginTop: 24 }}>
                ...
                >
                  <h2 style={{ margin: 0 }}>
                    {selectedDistrict
                      ? `Danh sách tuyến đường ${selectedDistrict}`
                ...
                >
                  {routes.map((r, i) => {
                    const depotName =
                      r.depot?.name ||
                      depots.find(
                        (d) => d.id === r.depot_id || d.id === selectedDepot
                      )?.name ||
                      "Depot";
                    const dumpName =
                      r.dump?.name ||
                      dumps.find(
                        (d) => d.id === r.dump_id || d.id === selectedDump
                      )?.name ||
                      "Dump";
                    const assignedEmployee = personnel.find(
                      (p) => p.id === r.driver_id
                    );
                      ...
                      >
                        ...
                        >
                          <div style={{ flex: 1 }}>
                            ...
                            >
                              Hành trình {String.fromCharCode(65 + i)} ·{" "}
                            ...
                            >
                              <strong>Số lượng bộ/tuyến:</strong>{" "}
                              {r.stops?.length || 0} điểm thu gom
                            </div>
                            <div
                              style={{
                                fontSize: 11,
                            ...
                            >
                              ...
                              {r.distance && r.distance > 0
                                ...
                                : "0.00"}
                              km · Thời gian: {r.eta || "00:00"} · Điểm dừng:{" "}
                              {r.stops?.length || 0}
                            </div>
                ...
                {activeRouteId && (
                  ...
                  >
                    💡 Nhấn đúp vào phương tiện khác để xem tuyến đường của
                    phương tiện đó
                  </div>
                )}
              </div>
            ...
            {selectedRoute && (
              ...
              >
                ...
                <div style={{ fontSize: 12, color: "#666" }}>
                  {selectedRoute.stops?.length || 0} điểm thu gom ·{" "}
            ...
            >
              ...
              {personnel.map((emp) => (
                <option key={emp.id} value={emp.id}>
                  {emp.name} ({emp.role === "driver" ? "Tài xế" : "Công nhân"})
                  - {emp.phone || "N/A"}
                </option>
              ))}
            </select>
