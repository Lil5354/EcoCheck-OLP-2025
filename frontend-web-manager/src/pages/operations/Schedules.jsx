...
export default function Schedules() {
  console.log('🚀 Schedules component mounted/re-rendered!')
  ...
  useEffect(() => {
    console.log('🔄 useEffect triggered - calling loadSchedules()')
    loadSchedules()
    loadPersonnel()
  }, [filterStatus])
  ...
  async function loadPersonnel() {
    try {
      ...
      console.log('📦 Personnel API response:', result)
      
      if (result.ok) {
        // Filter only driver and collector roles
        // Note: API returns 'role' field, not 'personnel_role'
        const allPersonnel = Array.isArray(result.data) ? result.data : []
        console.log('📋 All personnel:', allPersonnel.length, 'items')
        
        const filtered = allPersonnel.filter(
          p => p.role === 'driver' || p.role === 'collector'
        )
        console.log('✅ Filtered personnel (driver/collector):', filtered.length, 'items')
        console.log('👥 Personnel list:', filtered)
        setPersonnel(filtered)
      } else {
        console.error('❌ Error loading personnel:', result.error)
        setToast({ message: result.error || 'Lỗi khi tải danh sách nhân viên', type: 'error' })
      }
    } catch (error) {
      console.error('💥 Error loading personnel:', error)
      setToast({ message: 'Lỗi khi tải danh sách nhân viên: ' + error.message, type: 'error' })
    }
  }
  ...
  async function handleSaveAssignment() {
    ...
    try {
      console.log('Assigning employee:', { scheduleId: selectedSchedule.id, employeeId: selectedEmployeeId })
      const result = await api.updateSchedule(selectedSchedule.id, {
      ...
      if (result.ok) {
        ...
        await loadSchedules() // Reload danh sách
      } else {
  ...
  async function loadSchedules() {
    setLoading(true)
    try {
      console.log('🔍 Starting loadSchedules...', { filterStatus })
      const params = {}
      if (filterStatus) {
      ...
      console.log('📦 LoadSchedules response:', result)
      
      if (result.ok) {
        const schedulesData = Array.isArray(result.data) ? result.data : []
        console.log('✅ Setting schedules data:', schedulesData.length, 'items')
  ...
  const columns = [
    { 
      ...
      render: (r) => (
        <div>
          <div style={{ fontWeight: 500 }}>{r.citizen_name || 'N/A'}</div>
          <div style={{ fontSize: 12, color: '#666' }}>{r.citizen_phone || '-'}</div>
        </div>
      )
    },
    { 
    ...
    { 
      key: 'address', 
      label: 'Địa chỉ',
      render: (r) => r.address || (r.latitude && r.longitude ? `${r.latitude.toFixed(5)}, ${r.longitude.toFixed(5)}` : '-')
    },
    { 
      key: 'waste_type', 
      label: 'Loại rác',
      render: (r) => {
        ...
        return typeMap[r.waste_type] || r.waste_type
      }
    },
    { 
      key: 'estimated_weight', 
      label: 'Khối lượng (kg)',
      render: (r) => r.estimated_weight ? `${r.estimated_weight} kg` : '-'
    },
    { 
      key: 'status', 
      label: 'Trạng thái',
            ...
            {selectedSchedule && (
              <div style={{ marginTop: 16, padding: 12, backgroundColor: '#f5f5f5', borderRadius: 6 }}>
                ...
                <div style={{ fontSize: 14 }}>
                  <strong>{selectedSchedule.citizen_name}</strong><br/>
