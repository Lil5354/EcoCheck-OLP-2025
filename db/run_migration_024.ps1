# PowerShell script to run migration 024: Seed test schedules for today
# MIT License - Copyright (c) 2025 Lil5354

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  Migration 024: Seed test schedules for today" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# Migration file path
$migrationFile = "db/migrations/024_seed_today_schedules.sql"
...
if ($dockerAvailable) {
    ...
    if ($postgresRunning) {
        Write-Host "PostgreSQL container is running: ecocheck-postgres" -ForegroundColor Green
        
        # Run migration via Docker
        Write-Host "Running migration..." -ForegroundColor Yellow
        $env:PGPASSWORD = $DB_PASSWORD
        
        docker exec -i ecocheck-postgres psql -U $DB_USER -d $DB_NAME -f /app/$migrationFile
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "  - Created 15 completed schedules for today" -ForegroundColor Gray
            Write-Host "  - Created 3 scheduled (not completed) schedules for today" -ForegroundColor Gray
            Write-Host "  - All schedules have actual_weight for Dashboard display" -ForegroundColor Gray
            Write-Host ""
        } else {
            Write-Host ""
